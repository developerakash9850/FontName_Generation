...
//		Bitmap bwBitmap = ConvertToBlackAndWhite(bitmap);
	...
	private void CopyBufToStructBMP(byte[] RawBuf) 
	{
        ...
        //4-bytes
        Totalsize = 256 * (256 * (256 * RawBuf[idx+3] + RawBuf[idx+2]) + RawBuf[idx+1]) + RawBuf[idx];
        idx = idx + 4;

        //4-bytes
        Reserved = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;

        //4-bytes
        Byte_Offset = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;
        //4-bytes
        Size_infoheader = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;
        //4-bytes
        nHorizontalLen = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;
        //4-bytes
        nVeritcalLen = (256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx]) & 0x000000FF;
        idx = idx + 4;
        ...
        //4-bytes
        compressionType = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;
        //4-bytes
        actNoOfBytes = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;
        //4-bytes
        HzPixelPerMeter = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;
        //4-bytes
        VtPixelPerMeter = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;

        //4-bytes
        NoOfColorUsed = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;

        //4-bytes
        NoOfImptColor = 256 * (256 * (256 * RawBuf[idx + 3] + RawBuf[idx + 2]) + RawBuf[idx + 1]) + RawBuf[idx];
        idx = idx + 4;
	}
